...
from model import LengthModel, LengthModelConfig
...
from sklearn.linear_model import Ridge  # 과적합 방지용
...
def build_parser():
    ...
    p.add_argument("--eval_epoch", type=int, default=1, help="Evaluation epoch interval")
    ...
    # for statistics
    p.add_argument("--print_hist", type=str2bool, default=False, help="Whether to print histogram of lengths")
    p.add_argument("--num_bins", type=int, default=50, help="Number of bins for histogram")
...
def validate_model(
    ...
    Args:
        ...
        debugging: 디버깅 모드
        input_lengths: (Optional) 입력 길이 리스트 (디버깅용)
        target_lengths: (Optional) 타겟 길이 리스트 (디버깅용
    ...
    if total_samples > num_samples_to_use:
    ...
    if args.print_hist:
        ...
        Lin = np.array(input_lengths, dtype=np.float32)
        Lout = np.array(target_lengths, dtype=np.float32)
        diff = Lout - Lin

        x = Lin.reshape(-1, 1)
        deg = 2  # 2~3부터 시작 추천
        reg_model = make_pipeline(PolynomialFeatures(degree=deg, include_bias=False),
                                  Ridge(alpha=1.0))
        reg_model.fit(x, diff)

        def predict_len_poly(Lin_vals):
            Lin_arr = np.asarray(Lin_vals, dtype=np.float32)
            if Lin_arr.ndim == 0:
                x = Lin_arr.reshape(1, 1)
                d_hat = float(reg_model.predict(x)[0])
                return int(round(float(Lin_arr) + d_hat))
            x = Lin_arr.reshape(-1, 1)
            d_hat = reg_model.predict(x).reshape(-1)
            return np.rint(Lin_arr.reshape(-1) + d_hat).astype(int)
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            ...
            predicted_lengths = model(
                audio_feats, audio_feat_mask,
                text_feats, text_feat_mask        
            )
            
            predicted_length_ids = torch.argmax(predicted_lengths, dim=-1)  # B,
            hyp_lengths.extend(predicted_length_ids.long().cpu().tolist())  # B,
            target_lengths.extend(x1_mask.sum(dim=1).long().cpu().tolist())  # B,
            target_ids.extend(gts.long().cpu().tolist())
            if args.print_hist:
                stat_predict = predict_len_poly(hyp_mask.sum(dim=1).long().cpu().numpy())                
                stat_hyp_lengths.extend(stat_predict.tolist())  # B,                
    
    assert num_samples_to_use == len(hyp_lengths) == len(target_lengths)
    correct_predictions = 0    
    short_predictions = 0
    long_predictions = 0
    diff_lengths = []
    stat_correct_predictions = 0
    stat_diff_lengths = []
    for b in range(num_samples_to_use):
        hyp_length = hyp_lengths[b]
        target_length = target_lengths[b]        
        
        if hyp_length == target_length:            
            ...
            if hyp_length < target_length:
                ...
                long_predictions += 1
        diff_lengths.append(hyp_length - target_length)
        
        if args.print_hist:
            if stat_hyp_lengths[b] == target_length:
                stat_correct_predictions += 1
            stat_diff_lengths.append(stat_hyp_lengths[b] - target_length)
    
        if debugging:
            logger.info(f"Target IDs: {target_ids[b][:target_length]}")
            logger.info(f"Sample {b}: Target length = {target_length}, Predicted length = {hyp_length}")            
            if args.print_hist:
                logger.info(f"Stat Predicted length = {stat_hyp_lengths[b]}")
            logger.info("---------------------------")
    
    accuracy = correct_predictions / num_samples_to_use    
    variance = torch.var(torch.tensor(diff_lengths, dtype=torch.float)).item()
    std = math.sqrt(variance)    

    assert correct_predictions + short_predictions + long_predictions == num_samples_to_use
    logger.info(f"Accuracy: {accuracy * 100:.4f}% ({correct_predictions}/{num_samples_to_use})")
    logger.info(f"Short predictions: {short_predictions}")
    logger.info(f"Long predictions: {long_predictions}")
    logger.info(f"Length Prediction Variance: {variance:.4f}")    
    logger.info(f"Length Prediction Standard Deviation: {std:.4f}")

    if args.print_hist:
        stat_accuracy = stat_correct_predictions / num_samples_to_use
        stat_variance = torch.var(torch.tensor(stat_diff_lengths, dtype=torch.float)).item()
        stat_std = math.sqrt(stat_variance)
        logger.info(f"Stat Correct predictions: {stat_correct_predictions}/{num_samples_to_use}")
        logger.info(f"Stat Accuracy: {stat_accuracy * 100:.4f}%")
        logger.info(f"Stat Length Prediction Variance: {stat_variance:.4f}")
        logger.info(f"Stat Length Prediction Standard Deviation: {stat_std:.4f}")
    ...
    for epoch in range(init_epoch, args.final_epoch + 1):        
        ...
        for batch in train_loader:
            ...
            with torch.amp.autocast('cuda', enabled=False):
                # logits B, T, K
                logits = model(audio_feats=audio_feats,
                               audio_mask=audio_feat_mask,
                               text_feats=text_feats,
                               text_mask=text_feat_mask)

                lengths = x1_mask.sum(dim=1).long()  # B,
                length_loss = length_criterion(logits, lengths)
            ...
            if args.print_hist:
                target_lengths.extend(gt_mask.sum(dim=1).long().cpu().tolist())  # B,
                input_lengths.extend(hyp_mask.sum(dim=1).long().cpu().tolist())  # B,
                diff_lengths_batch = (hyp_mask.sum(dim=1) - gt_mask.sum(dim=1)).long().cpu().tolist()
                diff_lengths.extend(diff_lengths_batch)
                bucket_key = (hyp_mask.sum(dim=1) - gt_mask.sum(dim=1)).long().cpu().tolist()
                for i, key in enumerate(bucket_key):
                    bucket[key].append(loss_val)
        ...
        if args.print_hist:
            plt.hist(target_lengths, bins=args.num_bins, alpha=0.5, label='Target Lengths')
            plt.hist(input_lengths, bins=args.num_bins, alpha=0.5, label='Input Lengths')
            plt.xlabel('Length')
            plt.ylabel('Frequency')
            plt.title(f'Histogram of Target and Input Lengths at Epoch {epoch}')
            plt.legend()
            plt.savefig(os.path.join(args.save_dir, f'length_hist_epoch{epoch}.png'))
            plt.close()

            # print diff_lengths histogram
            plt.hist(diff_lengths, bins=args.num_bins, alpha=0.7, color='gray')
            plt.xlabel('Length Difference (Input - Target)')
            plt.ylabel('Frequency')
            plt.title(f'Histogram of Length Differences at Epoch {epoch}')
            plt.savefig(os.path.join(args.save_dir, f'length_diff_hist_epoch{epoch}.png'))
            plt.close()
