...
from config import ModelConfig, SLUConfig
...
class SLUModel(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.config = config
        self.text_model_name = self.config.model.llm_name
        ...
        self.loss = nn.CTCLoss(blank=self.config.model.blank_token_id,
                               zero_infinity=True)
    ...
    def forward(self, input_ids, input_masks,
        ...
        with torch.amp.autocast('cuda', enabled=torch.cuda.is_available()):
            ...
            if self.config.model.use_log_probs:
    ...
    @torch.no_grad()
    def greedy_search(self, x, masks):
        device = x.device
        with torch.inference_mode():
            text_embeddings = self.text_encoder(
                input_ids=x,
        ...
        # Upsample text embeddings if upsampler is provided
        if "upsampler" in self.components:
            upsampled_embeddings, _ = self.upsampler(
                text_embeddings,
                masks)
        text_embeddings = upsampled_embeddings
