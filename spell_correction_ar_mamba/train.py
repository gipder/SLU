...
from model import ARMambaModel, ARMambaModelConfig
...
def build_parser():
    ...
    p.add_argument("--use_mamba", type=str2bool, default=True, help="Whether to use Mamba SSM in ARMambaModel")
...
def validate(
    ...
    Returns:
        dict: 'ar_wer', 'asr_wer', 'gt_wer', 'accuracy' 포함
    """
    ar_model.eval()
    ...
    assert num_samples_to_use == len(id_hyps)
    for b in range(num_samples_to_use):
        hyp_ids = id_hyps[b]
        target_ids = id_targets[b]
        str_asr_hyp = str_asr_hyps_from_batch[b]
        str_gt = str_gts_from_batch[b]
    ...
    # Compute metrics
    ar_wer = wer(str_targets, str_hyps)
    ...
    logger.info(f"Correction WER: {ar_wer * 100:.4f}%")
...
def train_ar(
    ...
    device = (
        next(ar_model.parameters()).device
        if not isinstance(ar_model, torch.nn.DataParallel) 
        else next(ar_model.module.parameters()).device
    )
    ...
    for epoch in range(init_epoch, args.final_epoch + 1):        
        ...
        for batch in train_loader:
            ...
            B = gts.size(0)
            T = gts.size(1)            
            target_lengths = gt_mask.sum(dim=1)  # B,
            input_ids = torch.zeros((B, T+1), dtype=gts.dtype, device=device)
            input_ids[:, 1:] = gts[:, :]
            input_ids[:, 0] = sos_id  # sos token
            
            target_ids = torch.zeros((B, T+1), dtype=gts.dtype, device=device)
            target_ids[:, :-1] = gts[:, :]
            target_ids[torch.arange(B), target_lengths] = eos_id  # eos token

            gt_mask = torch.concat((torch.ones((B, 1), dtype=gt_mask.dtype), gt_mask), dim=1)  # B, T+1
            
            with torch.amp.autocast('cuda', enabled=False):
                # logits B, T, K
                logits = ar_model(input_ids=input_ids,
                                  audio_feats=audio_feats,
                                  text_feats=text_feats,
                                  audio_mask=audio_feat_mask,                                  
                                  text_mask=text_feat_mask)
                ...
                if args.loss_type == "ce":
                    # ce loss                    
                    corrupt_mask = (gt_mask == True).to(device)  # B, T_out
                    logits_perm = logits.permute(0, -1, 1)
                    ce_loss = criterion(logits_perm, target_ids)
                    mask = corrupt_mask.float()
                    denom = mask.sum().clamp_min(1.0)
                    ce_loss = (ce_loss * mask).sum() / denom
...
if __name__ == "__main__":
    ...
    logger.info(f"Tokenizer bos_token: {tokenizer.bos_token}, eos_token: {tokenizer.eos_token}")
    ...
    logger.info(f"* ModelConfig: {class_name(cfg)}")
    logger.info(json.dumps(asdict(cfg), indent=2))
        
    ar_model = ARMambaModel(cfg)
