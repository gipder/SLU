...
def extract_and_save_hubert_features(
    ...
    for start in range(0, len(wav_files), batch_size):
        ...
        #waves = [load_audio_16k(f"{audio_file_prefix}/{p}", target_sr=sr) for p in chunk]
        # feature_extractor로 패딩 + attention_mask 생성 (torch tensor로)
        ...
        # text normalize
        decoupled_normalized_seqlogical = manifest["decoupled_normalized_seqlogical"].copy()
        seqlogical = manifest["seqlogical"].copy()
        for i in range(len(transcriptions)):
            transcriptions[i] = normalize_text(transcriptions[i])
            ground_truth[i] = normalize_text(ground_truth[i])
            decoupled_normalized_seqlogical[i] = decoupled_normalized_seqlogical[i].lower()
            seqlogical[i] = seqlogical[i].lower()
...
if __name__ == "__main__":
    ...
    for key in manifest.keys():
        cache_dir = f"./hubert_deberta_cache"
