...
class MaskedLengthPredictionModule(nn.Module):
    ...
    def forward(
        ...
        predicted_lengths_logits = self.lengths_pred(lengths_enc)
        # FP16 대비 log_softmax는 float32로
        predicted_lengths = F.log_softmax(
            predicted_lengths_logits.float(), dim=-1
        ).to(predicted_lengths_logits.dtype)
        return predicted_lengths, predicted_lengths_logits
...
if __name__ == "__main__":
    ...
    log_probs, logits = model(x, padding_mask)
    log_probs2, logits2 = model2(x, padding_mask)
    # parameter count
